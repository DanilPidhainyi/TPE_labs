...
def get_y(x1, x2, x3):
    return 2.4 + 0.5 * x1 + 6.0 * x2 + 10.0 * x3 + 4.2 * x1 * x1 + 0.9 * x2 * x2 + 2.2 * x3 * x3\
           + 0.8 * x1 * x2 + 0.9 * x1 * x3 + 7.2 * x2 * x3 + 4.7 * x1 * x2 * x3 + randrange(0, 10) - 5
...
Y = [[get_y(list_for_a[j][0], list_for_a[j][1], list_for_a[j][2]) for i in range(m)] for j in range(15)]
...
for i in range(n):
    ...
    print("")
# середні у
Y_average = [np.mean(Y[i], axis=0) for i in range(len(Y))]
...
F4 = n - d
print("Fp =", Fp)
if Fp < f.ppf(q=0.95, dfn=F4, dfd=F3) and coefs1 != 2:
